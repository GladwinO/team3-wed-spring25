...
# Add this custom formset class
class BaseListingSlotFormSet(BaseInlineFormSet):
    def clean(self):
        """Validate at least one slot remains"""
        super().clean()
        
        # Count non-deleted forms
        non_deleted = 0
        first_form_deleted = False
        
        for i, form in enumerate(self.forms):
            if form.cleaned_data and not form.cleaned_data.get('DELETE', False):
                non_deleted += 1
            elif i == 0 and form.cleaned_data and form.cleaned_data.get('DELETE', False):
                # First form is being deleted
                first_form_deleted = True
                form.cleaned_data['DELETE'] = False  # Prevent deletion
        
        if first_form_deleted:
            raise forms.ValidationError("Cannot delete the first time slot.")
            
        if non_deleted == 0:
            raise forms.ValidationError("At least one time slot is required.")


# Update your formset definition
ListingSlotFormSet = inlineformset_factory(
    Listing, 
    ListingSlot, 
    form=ListingSlotForm, 
    formset=BaseListingSlotFormSet,  # Use our custom formset
    extra=0,  # No extra forms by default
    can_delete=True  # Keep this to allow deleting non-first slots
)
